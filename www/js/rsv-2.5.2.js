...
// SETTINGS
rsv.displayType          = "display-html"; // "alert-one", "alert-all" or "display-html"
...
rsv.processError = function(obj, message)
{
  ...
  switch (rsv.displayType)
  {
    ...
    case "display-html":
      var found = false;
      for(var i in rsv.returnHash){
        if (rsv.returnHash[i][1] == message){
          found = true;
          break;
        }
      }
      if (!found){
        ...
        obj.onkeypress = function(){
          this.className = this.className.replace(new RegExp("\\b" + rsv.errorFieldClass + "\\b"), '');
        }
      }
      break;
  }
